...
import type { Book } from '../features/books/books_types';
  ...
  const filteredBooks = useMemo(() => {
    return books.filter((book: Book) => {
      const matchesCategory =
        !filters.category_id || book.category?.id.toString() === filters.category_id;
      const matchesSearch =
        !filters.search ||
    ...
    <div className="space-y-10 pb-12">
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            className={`px-6 py-3 rounded-full text-xs font-black uppercase border ${
              !filters.category_id ? 'bg-primary text-white' : 'bg-white text-gray-400'
            }`}
          >
            All Collections
          </button>
          ...
          {categories.map(cat => (
            <button
              ...
              className={`px-6 py-3 rounded-full text-xs font-black uppercase border ${
                filters.category_id === cat.id.toString() ? 'bg-primary text-white' : 'bg-white text-gray-400'
              }`}
            >
              {cat.name}
            </button>
      ...
      {deleteModalOpen && (
        <DeleteConfirmationModal
          ...
          title="Delete Book?"
          message="Are you sure you want to delete this book permanently? This action cannot be undone."
